    ...
    >
      {secondarySpouses.map((spouse) => (
        <React.Fragment key={`secondary_${pair[0]?.id || 'lead'}_${spouse.id}`}>
          <button
            type="button"
            className={`${toCardClass(spouse, selectedMemberId, cardClassName)} ${cardSizeClassName}`.trim()}
            onClick={() => onSelectMember && onSelectMember(spouse.id)}
          >
            <MemberPhoto member={spouse} />
            <strong>{spouse.date_of_death ? `${formatDisplayName(spouse.name)} ז"ל` : formatDisplayName(spouse.name)}</strong>
      ...
      {pair.map((member, index) => (
        <React.Fragment key={member.id}>
          <button
            type="button"
            className={`${toCardClass(member, selectedMemberId, cardClassName)} ${cardSizeClassName}`.trim()}
            data-descendant-anchor={index === 0 ? 'true' : undefined}
            onClick={() => onSelectMember && onSelectMember(member.id)}
          >
            <MemberPhoto member={member} />
            <strong>{member.date_of_death ? `${formatDisplayName(member.name)} ז"ל` : formatDisplayName(member.name)}</strong>
...
export default function FamilySchematicCanvas({ members, selectedMemberId, onSelectMember }) {
  ...
  return (
    <section className="schematic-canvas-root">
